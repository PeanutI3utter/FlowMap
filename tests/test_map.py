...
def test_map0():
    ...
    for node in res.nodes:
        ...
        assert res.nodes[node]['label'] == expected.nodes[node]['label']
        assert res.nodes[node]['gtype'] == expected.nodes[node]['gtype']
        assert res.nodes[node]['func'].equiv(expected.nodes[node]['func'])
